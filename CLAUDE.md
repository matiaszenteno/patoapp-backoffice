...
- La lógica pura (`src/lib/`) se testea con `npm test` (`node --test`). Una sola vez antes de push/PR, ejecutar `npm run validate`: tests, typecheck y bundle. CI corre el mismo comando mediante el job estable `test-gate` en cada PR y sobre el SHA integrado en `main`; sólo después de ese gate el deploy reconstruye el bundle, sin repetir tests ni typecheck. Los componentes no tienen tests: los cambios de UI se validan con `npm run dev` + navegación manual.
